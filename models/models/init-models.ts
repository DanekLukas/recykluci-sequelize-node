...
export function initModels(sequelize: Sequelize) {
  ...
  basket.belongsTo(data, { as: "datum", foreignKey: "data_id"});
  data.hasMany(basket, { as: "baskets", foreignKey: "data_id"});
  homepage.belongsTo(data, { as: "datum", foreignKey: "data_id"});
  data.hasMany(homepage, { as: "homepages", foreignKey: "data_id"});
  sells_item.belongsTo(data, { as: "datum", foreignKey: "data_id"});
  data.hasMany(sells_item, { as: "sells_items", foreignKey: "data_id"});
  sells.belongsTo(delivery, { as: "delivery", foreignKey: "delivery_id"});
  delivery.hasMany(sells, { as: "sells", foreignKey: "delivery_id"});
  data.belongsTo(image, { as: "id_image", foreignKey: "id"});
  image.hasOne(data, { as: "datum", foreignKey: "id"});
  sells_item.belongsTo(sells, { as: "sell", foreignKey: "sells_id"});
  sells.hasMany(sells_item, { as: "sells_items", foreignKey: "sells_id"});
  data.belongsTo(size, { as: "size", foreignKey: "size_id"});
  size.hasMany(data, { as: "data", foreignKey: "size_id"});
  delivery.belongsTo(size, { as: "size", foreignKey: "size_id"});
  size.hasMany(delivery, { as: "deliveries", foreignKey: "size_id"});
